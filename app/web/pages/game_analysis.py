...
def _bar_row(
    sym: str, name: str, pct: float, val_str: str, fill: Optional[str] = None
) -> str:
...
def _counts_row(
    sym: str, name: str, items: list[tuple[Optional[int], str, str]]
) -> str:
...
def _render_stockfish_html(
    ...
    w_acc: float,
    b_acc: float,
    w_acpl: Optional[float],
    b_acpl: Optional[float],
    w_bril: Optional[int],
    b_bril: Optional[int],
    w_best: Optional[int],
    b_best: Optional[int],
    w_great: Optional[int],
    b_great: Optional[int],
    w_inac: Optional[int],
    b_inac: Optional[int],
    w_mist: Optional[int],
    b_mist: Optional[int],
    ...
    return (
        ...
        f"</div>" + acc_section + acpl_section + quality_section + "</div>"
    )
...
def _render_lc0_html(
    ...
    if any(v is not None for v in [w_inac, w_mist, w_blun, b_inac, b_mist, b_blun]):
        ...
        errors_section = (
            f'<hr class="dub-rule">'
            f'<div class="dub-lbl">Move Errors</div>' + w_row + b_row
        )
    ...
    return (
        ...
        f'<span class="dub-meta">{escape(meta)}</span>'
        f"</div>" + wdl_section + errors_section + "</div>"
    )
...
def _count_classified_moves(
    moves_df, white_to_move: bool, classification: str
) -> Optional[int]:
...
def _engine_queue_status(game_id: str, engine: str) -> Optional[str]:
...
def _derive_side_stats(
    moves_df, white_to_move: bool
...
def _player_label(name: str, rating: Optional[int]) -> str:
...
if _game_slug:
    ...
    st.caption(
        f"Game ID: [{analysis.game_id}](/game-analysis?game_id={analysis.game_id})"
    )

# ── Opening label ─────────────────────────────────────────────────────────────
if analysis.eco_code or analysis.opening_name:
    _op_name = opening_display_label(
        analysis.eco_code, analysis.lichess_opening, analysis.opening_name, analysis.pgn
    )
    _op_href = (
        f"/opening-position?opening_id={analysis.opening_id}"
        if analysis.opening_id
...
white_blunders = (
    ...
    if analysis.white_blunders is not None
    else derived_white["blunders"]
)
white_mistakes = (
    analysis.white_mistakes
    if analysis.white_mistakes is not None
    else derived_white["mistakes"]
)
white_inaccuracies = (
    analysis.white_inaccuracies
    if analysis.white_inaccuracies is not None
    else derived_white["inaccuracies"]
)
black_blunders = (
    analysis.black_blunders
    if analysis.black_blunders is not None
    else derived_black["blunders"]
)
black_mistakes = (
    analysis.black_mistakes
    if analysis.black_mistakes is not None
    else derived_black["mistakes"]
)
black_inaccuracies = (
    analysis.black_inaccuracies
    if analysis.black_inaccuracies is not None
    else derived_black["inaccuracies"]
)

white_best_moves = _count_classified_moves(
    analysis.moves, white_to_move=True, classification="best"
)
black_best_moves = _count_classified_moves(
    analysis.moves, white_to_move=False, classification="best"
)
white_brilliant_moves = _count_classified_moves(
    analysis.moves, white_to_move=True, classification="brilliant"
)
black_brilliant_moves = _count_classified_moves(
    analysis.moves, white_to_move=False, classification="brilliant"
)
white_great_moves = _count_classified_moves(
    analysis.moves, white_to_move=True, classification="great"
)
black_great_moves = _count_classified_moves(
    analysis.moves, white_to_move=False, classification="great"
)
...
if missing_lc0 or missing_sf:
    ...
    if missing_lc0 and missing_sf:
        st.info(
            "No engine analysis yet for this game. Scroll to the Admin section at the bottom to queue analysis."
        )
    elif missing_lc0:
        st.info(
            "Lc0 analysis is not yet available for this game. Scroll to the Admin section to queue it."
        )
    else:
        st.info(
            "Stockfish analysis is not yet available for this game. Scroll to the Admin section to queue it."
        )
...
if lc0_ready:
    ...
    wdl_data = analysis.lc0_moves[
        [c for c in wdl_cols if c in analysis.lc0_moves.columns]
...
if "cp_eval" in analysis.moves.columns and analysis.moves["cp_eval"].notna().any():
    sf = analysis.moves[
        ["ply", "cp_eval"]
        + [c for c in ["san", "classification"] if c in analysis.moves.columns]
    ].dropna(subset=["cp_eval"])
...
with st.expander("Admin", expanded=False):
        ...
        if show_lc0_queue:
            with q_col_lc0:
                if st.button(
                    ...
                    queued = enqueue_game(
                        game_id, engine="lc0", depth=_settings.lc0_nodes
                    )
                    if queued:
                        _set_queue_flash(
                            "success",
                            "Queued for Lc0 analysis. Start the worker to process it.",
                        )
                    else:
        ...
        if show_sf_queue:
            with q_col_sf:
                if st.button(
                    ...
                    queued = enqueue_game(
                        game_id, engine="stockfish", depth=_settings.analysis_depth
                    )
                    if queued:
                        _set_queue_flash(
                            "success",
                            "Queued for Stockfish analysis. Start the worker to process it.",
                        )
                    else:
    ...
    if any_reanalyze:
        ...
        st.caption(
            "Queuing reanalysis will overwrite all prior engine output for this game."
        )
            ...
            with re_col_lc0:
                if st.button(
                    ...
                    if queued:
                        _set_queue_flash(
                            "success",
                            "Queued Lc0 reanalysis. New output will replace the current analysis.",
                        )
                    else:
            ...
            with re_col_sf:
                if st.button(
                    ...
                    if queued:
                        _set_queue_flash(
                            "success",
                            "Queued Stockfish reanalysis. New output will replace the current analysis.",
                        )
                    else:
                        _set_queue_flash(
                            "info", "Stockfish is already queued for this game."
                        )
                    st.rerun()

    if not any_queue and not any_reanalyze:
        st.info(
            "Analysis is currently in progress for this game. Buttons will appear here once the queue is clear."
        )
